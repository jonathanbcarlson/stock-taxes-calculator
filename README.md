...
- **Direct purchase price input** — enter the price per share from your ESPP confirmation; no need to know your plan's lookback details
...
- **Qualifying ordinary income (IRC §423(c)):** `min(salePrice − pricePaid, discount% × offeringFMV)` — the cap is the plan discount measured at the **offering/grant date**, not `offeringFMV − pricePaid`
...
- **Plan discount** is configurable and defaults to 15% — the statutory maximum under §423(b)(6). It only affects the qualifying-disposition cap; the disqualifying scenarios are computed from the prices alone. `calcScenario` takes the same value as a `discountRate` parameter
...
`discount% × (offeringFMV − purchaseFMV)` per share, so the penalty is real but small — and in
